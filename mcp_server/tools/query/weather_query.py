...
from mcp_server.tools.tool_base import ToolBase, OperationConfig, ToolResult
...
class WeatherQueryTool(ToolBase):
    ...
    OPERATION_CONFIG = {
        "ip_weather": OperationConfig(
            description="根据IP地址查询天气",
            required_params=[],
            optional_params=["ip", "days", "day", "suntimetype", "dkey", "uip"]
        ),
        "domestic_weather": OperationConfig(
            description="查询国内城市天气",
            required_params=["province", "city"],
            optional_params=["days", "day", "suntimetype", "dkey", "uip"]
        ),
        "foreign_weather": OperationConfig(
            description="查询国外城市天气",
            required_params=["city"],
            optional_params=["days", "day", "dkey", "uip"]
        )
    }
    ...
    def validate_parameters(self, operation: str, **kwargs) -> Tuple[Dict[str, Any], Optional[str]]:
        ...
        if operation == WeatherOperationEnum.DOMESTIC_WEATHER:
            if not kwargs.get('province'):
                return params, "国内天气查询需要提供province参数"
            elif not kwargs.get('city'):
                return params, "国内天气查询需要提供city参数"
        elif operation == WeatherOperationEnum.FOREIGN_WEATHER:
            if not kwargs.get('city'):
                return params, "国外天气查询需要提供city参数"
    ...
    def parse_hourly_temperature(self, hourly_data: list) -> tuple:
        """从小时预报数据中解析日内最高温和最低温
    ...
    def _format_weather_response(self, data: Dict[str, Any], location_info: str, days: int = 1, day: int = None, suntimetype: int = None) -> Dict[str, Any]:
        """格式化天气响应数据
        ...
        day_temp_high, day_temp_low = self.parse_hourly_temperature(hourly_data)
        ...
        if day is not None and day != 1:
            ...
            if suntimetype == 1:
                ...
                if sunrise_sunset and day <= len(sunrise_sunset):
                    day_info = sunrise_sunset[day - 1]
                    result_data["sunrise_sunset"] = [day_info]
        else:
            ...
            if data.get('alarm'):
                ...
                if isinstance(alarm_data, dict):
                    ...
                    if title:
                        ...
                        formatted_message += f"\n{alarm_message}"
                        result_data["alarm"] = alarm_message
                ...
                if sunrise_sunset:
                    ...
                    if day is not None:
                        ...
                        for day_info in sunrise_sunset[:days]:
                            date_display = day_info.get('date_formatted', '') or day_info.get('date', '')
                            formatted_message += f"• {date_display}: 日出 {day_info.get('sunrise', '')}, 日落 {day_info.get('sunset', '')}, 白昼 {day_info.get('daytime', '')}, 夜晚 {day_info.get('nighttime', '')}\n"
                        result_data["sunrise_sunset"] = sunrise_sunset[:days]
    
        # 构建完整结果
        return ToolResult.success(result_data).with_message(formatted_message).with_blackboard(formatted_message).build()

    async def execute(self, ctx=None, **kwargs) -> Dict[str, Any]:
        """执行天气查询操作
        ...
        if operation == "ip_weather":
            return await self.query_ip_weather(
                ip=kwargs.get('ip'),
                days=kwargs.get('days', 1),
                day=kwargs.get('day'),
                suntimetype=kwargs.get('suntimetype'),
                dkey=kwargs.get('dkey'),
                uip=kwargs.get('uip')
            )
        elif operation == "domestic_weather":
            return await self.query_domestic_weather(
            ...
            return await self.query_foreign_weather(
                city=kwargs.get('city'),
                days=kwargs.get('days', 1),
                day=kwargs.get('day'),
                dkey=kwargs.get('dkey'),
            ...
            return ToolResult.config_error(f"不支持的操作类型: {operation}，支持的类型: ip_weather, domestic_weather, foreign_weather").build()
    
    async def query_ip_weather(self, ip: str = None, days: int = 1, day: int = None, suntimetype: int = None, dkey: str = None, uip: str = None) -> Dict[str, Any]:
        """根据当前IP地址自动查询天气信息
        ...
        Returns:
            天气查询结果字典
        """
        user_id, api_key = self.get_weather_api_config()
        
        # 使用统一的参数验证机制
        params, config_error = self.validate_parameters('ip_weather', ip=ip, days=days, day=day, suntimetype=suntimetype, dkey=dkey, uip=uip)
        ...
        day = params.get('day')
        suntimetype = params.get('suntimetype')
        
        # 如果没有提供IP，自动获取公网IP
        if not ip:
            ip = self.get_public_ip()
            if not ip:
                return ToolResult.error("无法获取公网IP地址，请检查网络连接").build()
        ...
        try:
            ...
            if data.get('code') == 200:
                ...
                return self._format_weather_response(data, location_info, days=days, day=day, suntimetype=suntimetype)
            else:
                return ToolResult.error(f"请求失败: {data.get('msg', '未知错误')}").build()
        except Exception as e:
            return ToolResult.error(f"API请求异常: {str(e)}").build()
    
    async def query_domestic_weather(self, province: str, city: str, days: int = 1, day: int = None, suntimetype: int = None, dkey: str = None, uip: str = None) -> Dict[str, Any]:
        """查询中国境内指定省份和城市的天气
        ...
        # 使用统一的参数验证机制
        params, config_error = self.validate_parameters('domestic_weather', province=province, city=city, days=days, day=day, suntimetype=suntimetype, dkey=dkey, uip=uip)
    ...
    async def query_foreign_weather(self, city: str, days: int = 1, day: int = None, dkey: str = None, uip: str = None) -> Dict[str, Any]:
        """查询国外主要城市的天气
        ...
        # 使用统一的参数验证机制
        params, config_error = self.validate_parameters('foreign_weather', city=city, days=days, day=day, dkey=dkey, uip=uip)
                ...
                if forecast_list:
                    ...
                    if day is not None:
                        ...
                        if forecast_list and day <= len(forecast_list):
                            ...
                            return ToolResult.error(f"查询的day参数{day}超出范围，最大支持{len(forecast_list)}天").build()
                    else:
                        # 显示指定天数的天气
                        if days > 1:
                    ...
                    return ToolResult.error("未获取到天气数据").build()
            else:
                return ToolResult.error(f"请求失败: {data.get('msg', '未知错误')}").build()
        except requests.exceptions.Timeout:
            return ToolResult.error("API请求超时").build()
        except requests.exceptions.RequestException as e:
            return ToolResult.error(f"API请求异常: {str(e)}").build()
        except ValueError as e:
            return ToolResult.error(f"API返回数据格式错误: {str(e)}").build()
...
def register_weather_query_tools(mcp):
    ...
    async def weather_query(
        ...
        try:
            ...
            # 调用工具实例的execute方法
            return await weather_tool.execute(
                operation=operation,
                province=province,
                city=city,
                ip=ip,
            ...
            import traceback
            traceback.print_exc()
            return ToolResult.error(f"程序执行出错: {str(e)}").build()
